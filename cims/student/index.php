...
{	
	$ret = mysqli_query($con, "SELECT * FROM student WHERE email='" . $_POST['username'] . "' AND password='" .md5($_POST['password'])."'");
...
$status=1;
$log=mysqli_query($con,"insert into studentlog(sid,studentname,studentip,status) values('".$_SESSION['id']."','".$_SESSION['login']."','$uip','$status')");
...
$uip=$_SERVER['REMOTE_ADDR'];
$status=0;
mysqli_query($con,"insert into studentlog(studentname,studentip,status) values('".$_SESSION['login']."','$uip','$status')");
...
$query=mysqli_query($con,"SELECT * FROM student WHERE email='$email' and contactNo='$contact'");
$num=mysqli_fetch_array($query);
if($num>0)
{
mysqli_query($con,"update student set password='$password' WHERE email='$email' and contactNo='$contact' ");
